    ...
    <Box css={styles}>
      <svg viewBox='0 0 845.2 458' css={styles}>
        {points.map(([cx, cy]) => {
          return hl && hl.indexOf(`${cx}-${cy}`) > -1 ? (
            <>
              ...
              {/* <circle key={`${cx}-${cy}`} cx={cx} cy={cy} r='1.7' fill='none' stroke='#4BCA81' strokeWidth={0.5} /> */}
            </>
          ) : (
        ...
        <filter id='shadow' x='-100%' y='-100%' width='400%' height='400%'>
          <feDropShadow dx='0' dy='0' stdDeviation='8px' flood-color='#4BCA81'></feDropShadow>
          <feDropShadow dx='0' dy='0' stdDeviation='1px' flood-color='#4BCA81'></feDropShadow>
        </filter>
      </svg>
    </Box>
  )
